import { ArtworkModel } from './artwork.model';

export const ArtworkMocks: ArtworkModel[] = [
  {
    id: 'dfghjkiuyfgbnj',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 01',
  },
  {
    id: 'jg7ew878hubugh',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 02',
  },
  {
    id: 'gfgujhgtyy7uijbhyj',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 03',
  },
  {
    id: 'gfrt67uijnhytgy78uij',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 04',
  },
  {
    id: 'nhgt678uijnbytuj',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 05',
  },
  {
    id: 'nhgt678ijhgt67uj',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 06',
  },
  {
    id: 'hhgt678uijhyt78ujh',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 07',
  },
  {
    id: 'jnhy789oikjhy78ui',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 08',
  },
  {
    id: 'jhgy78uijhgty78ujn',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 09',
  },
  {
    id: 'mjnhgy78ijhgy7uj',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 10',
  },
  {
    id: 'nbgy89ijhy678iuj',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 11',
  },
  {
    id: 'nbgy78ijhgy78uij',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 12',
  },
  {
    id: 'hgy78iujhgty7uj',
    creatorId: 'jmngt67u8iujhgty7ujh',
    source:
      'https://images.unsplash.com/photo-1653059959899-70ab4e464401?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=80&raw_url=true&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774',
    description:
      'Löksås ipsum tid vi annan smultron både ta tre, dunge hans vad vid smultron häst lax, om nya kanske sin vemod sjö gör. Blev kom erfarenheter kom ta kan faktor hav där tid kunde på, vi har både sista blivit hav samma tidigare icke brunsås, groda blivit ordningens gamla som vemod hav hans fram där.',
    title: 'An Artwork 13',
  },
].map((simpleMock) => ({
  ...simpleMock,
  slug: simpleMock.title.toLowerCase().split(' ').join('-'),
}));
